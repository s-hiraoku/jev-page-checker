...
- 各項目のカードには、質問、判定、Jev の回答、その問と判定についての短い寸評、ページから選んだ単位を出す。引用には取得元と、Jev が選んだものか表示用に添えたものかを表示する。判定のチップは繰り返さず、基準の段落も貼らない。寸評はどの URL でも同じ文である。Jev にカードの文章を書かせない。
...
- Each item card shows the question, verdict, Jev's answer, a short client-side remark for that question and verdict, and a page span. The citation names its source and whether Jev selected it or the app attached it for display. Cards do not repeat the verdict chip or paste the criterion paragraph. The remark is the same for every URL. Jev does not write the card copy.
