...
export function choiceLabel(questionId: string, choice: string, locale: ResolvedLocale = "ja", definitionVersion?: number): string {
  ...
  if (choice === "pass") return locale === "ja" ? "適合" : "Pass";
...
const INSTRUCTION_JA: Record<string, string> = {
  ...
  separates_fact_and_opinion:
    "事実の記述と著者の判断を、読者が区別できるか。エッセイや意見のページは混ざってよい。判断が判断として読め、報じられた事実が創作でなければ基準を満たします。主張があるからといってエッセイを失敗にしません。推測だと、仮説だと、実験が必要だと記された設計は、計測した事実として書いた判断ではありません。判断を計測した事実として書いている場合、または出典を避けるために計測した事実を好みの問題として書いている場合は失敗です。",
  unsourced_specifics:
  ...
  self_consistent:
    "同じ事実について、本文の中で矛盾がないか。一致する二つの記述は基準を満たします。同じ主張の反復は矛盾ではありません。同じ事実に相容れない数字、日付、結果がある場合、または後で無かったことにしている場合は失敗です。",
  certainty_matches_evidence:
    "断定の強さは、ページが実際に示している根拠に見合っているか。強い主張には強い根拠が要ります。個人の結果だと記したもの、不確かだと和らげた点は基準を満たします。仮説、未検証、実験が必要と記した主張は、正しい不確かさです。確定事実として扱いません。健康、金、身元、法について確定したように聞こえる主張には、ページ上の確定した根拠が要ります。争点がある主張、または根拠が薄い主張を確定事実として述べている場合は失敗です。",
};

const CITE_INSTRUCTION_JA: Record<string, string> = {
  evidence_for_claims_cite:
    "このページから切った文が選択肢です。各ラベルの説明がその文です。確定した事実主張の根拠になっている文を一つ選んでください。名指しの出典、文書、データ、方法、または著者が自分の仕事だと示している文です。ページでいちばん具体的な段落だから、という理由では選ばないでください。この問が基準を満たさないときは、基準を満たす部分を支える文ではなく、失敗の原因になっている文を選んでください。その根拠に関わる文が無ければ none です。新しい文は書かないでください。",
  separates_fact_and_opinion_cite:
    "このページから切った文が選択肢です。各ラベルの説明がその文です。事実と意見の切り分けに関わる文を一つ選んでください。事実と著者の判断が並んでいる文、判断を計測した事実として書いた文、または計測した事実を好みとして書いた文です。数字や出典があるという理由だけでは選ばないでください。この問が基準を満たさないときは、基準を満たす部分を支える文ではなく、失敗の原因になっている文を選んでください。その切り分けに関わる文が無ければ none です。新しい文は書かないでください。",
  unsourced_specifics_cite:
    "このページから切った文が選択肢です。各ラベルの説明がその文です。数字、日付、引用、名指しの出来事が入っている文を一つ選んでください。確認できる出典が無く確定事実として出ている具体があればそれを優先し、無ければ記事を支える具体が入っている文です。出典の名前だけで具体が無い文は選ばないでください。この問が基準を満たさないときは、基準を満たす部分を支える文ではなく、失敗の原因になっている文を選んでください。具体に関わる文が無ければ none です。新しい文は書かないでください。",
  self_consistent_cite:
    "このページから切った文が選択肢です。各ラベルの説明がその文です。本文の内部矛盾に関わる文を一つ選んでください。同じ事実について別の文と食い違う数字、日付、結果の文、または後で取り消している主張の文です。食い違いが無ければ、別の文も述べている事実の文です。いちばん具体的な段落だから、という理由では選ばないでください。この問が基準を満たさないときは、基準を満たす部分を支える文ではなく、失敗の原因になっている文を選んでください。同じ事実に触れる文が無ければ none です。新しい文は書かないでください。",
  certainty_matches_evidence_cite:
    "このページから切った文が選択肢です。各ラベルの説明がその文です。断定の強さに関わる文を一つ選んでください。断定、和らげ、仮説、個人の結果のいずれかの文です。言い切りの強さと、その文の中の根拠がずれている文があればそれを優先します。数字があるという理由だけでは選ばないでください。この問が基準を満たさないときは、基準を満たす部分を支える文ではなく、失敗の原因になっている文を選んでください。断定の強さが問題になる文が無ければ none です。新しい文は書かないでください。",
  identifiable_publisher_cite:
    "ラベルは、ページに出ているタイトル、サイト名、著者、説明、そのあとに本文から切った文です。各ラベルの説明がその文字列です。責任者の名前が分かるもの、または責任者がいないと分かるものを一つ選んでください。この問が基準を満たさないときは、基準を満たす部分を支えるものではなく、失敗の原因になっているものを選んでください。責任者に関わるものが無ければ none です。新しい文は書かないでください。",
  honest_identity_cite:
    "ラベルは、ページに出ているタイトル、サイト名、著者、説明、そのあとに本文から切った文です。各ラベルの説明がその文字列です。自分として話しているか、別の主体を名乗っているかが分かるものを一つ選んでください。この問が基準を満たさないときは、基準を満たす部分を支えるものではなく、失敗の原因になっているものを選んでください。身元に関わるものが無ければ none です。新しい文は書かないでください。",
  site_purpose_cite:
    "ラベルは、ページに出ているタイトル、サイト名、著者、説明、そのあとに本文から切った文です。各ラベルの説明がその文字列です。このページが何のためにあるかが分かるものを一つ選んでください。この問が基準を満たさないときは、基準を満たす部分を支えるものではなく、失敗の原因になっているものを選んでください。目的に関わるものが無ければ none です。新しい文は書かないでください。",
  disclosed_incentives_cite:
    "ラベルは、ページに出ているタイトル、サイト名、著者、説明、そのあとに本文から切った文です。各ラベルの説明がその文字列です。勧誘、得をする主体の名前、または勧誘がないことが分かるものを一つ選んでください。この問が基準を満たさないときは、基準を満たす部分を支えるものではなく、失敗の原因になっているものを選んでください。勧誘に関わるものが無ければ none です。新しい文は書かないでください。",
};
