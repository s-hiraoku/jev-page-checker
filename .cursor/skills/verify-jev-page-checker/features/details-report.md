...
- **URL entry.** Open the pass details page. Run `control-jev browser goto --path "/?scene=pass#details"`. Meta `Report · v6` is visible. Lanes are both `Pass`. The title row is labeled `タイトル`, not `標題`. The `語数` row is `210 語`. The `HTTPS` row is `あり`. `公開日` is `2026-09-18`. `ページの形` is `記事`. `外部ホスト` includes `transport.example.gov`. The button `この端末にファイルで残す` is present. Group heads `サイト` and `本文` appear above the question tables. Question rows include Japanese rubric basis text and the publisher cite `Mina Ito · Example News`. The `主張の根拠` row includes `12 September`. `送った文` includes `The city transportation bureau said Thursday`. There is no History list on this page. The `処理` row is visible on this full report.
