...
- **URL entry.** Open the pass details page. Run `control-jev browser goto --path "/?scene=pass#details"`. The banner reads `Jev Audit v10`. The summary line is `サイト 通過 | 本文 通過 | 報道・事実報告`. The title row is labeled `タイトル`, not `標題`. The `語数` row is `210 語`. The `HTTPS` row is `あり`. `公開日` is `2026-09-18`. `ページの形` is `記事`. `外部ホスト` includes `transport.example.gov`. The button `この端末にファイルで残す` is present. Group heads `サイト` and `本文` appear above the cards. Each card is `判定`, `基準`, `寸評`, `根拠`. The publisher evidence is `Mina Ito`. The `主な主張に根拠があるか` card includes `12 September`. `送った文` includes `The city transportation bureau said Thursday`. There is no History list on this page. The `処理` row is visible on this full report.
