...
- **URL entry.** Open the pass details page. Run `control-jev browser goto --path "/?scene=pass#details"`. Meta `Report · v4` is visible. Lanes are both `Pass`. The title row is labeled `タイトル`, not `標題`. The extract row contains `210 語` and `HTTPS あり`. Group heads `サイト` and `本文` appear above the question tables. `送った文` includes `The city transportation bureau said Thursday`. There is no History list on this page.
