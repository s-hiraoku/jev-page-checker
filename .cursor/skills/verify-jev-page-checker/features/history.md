...
- **URL entry.** Open History on the pass scene. Run `control-jev browser goto --path "/?scene=pass#history"`. Meta `History · v7` is visible. The heading is `History`, not `履歴`. Both seeded titles are listed.
