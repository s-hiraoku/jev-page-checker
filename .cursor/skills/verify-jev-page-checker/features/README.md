# Jev 信憑性チェッカー verification map

This directory is the maintained source for verifying the user-facing preview of Jev 信憑性チェッカー. Read this index before driving, then use the matching feature file as the recipe.
...
- Run `control-jev doctor` and require `ok: true`, origin `http://127.0.0.1:4174`, title `Jev 信憑性チェッカー プレビュー`.
...
- After a mutation (`設定を保存`, scene change), read the next screen. Do not trust the click alone.
- Restore nothing on disk; preview state is memory. Cleanup the instance, keep artifacts.

## Proof and skip reporting

- Capture the user action and the resulting state, not only the final screen.
- UI proof includes an ARIA snapshot and a screenshot that shows `Jev 信憑性チェッカー`.
